console.log("Background script loaded");chrome.runtime.onMessage.addListener((e,r,t)=>{if(e.action==="classifyPage")return e.useLocalModel?s(e.content,t):n(e.content,t),!0;if(e.action==="checkModelAvailability")return c().then(t),!0});async function c(){try{const e=await ai.languageModel.create();return console.log("Model availability check result:",e!=null),e!=null}catch(e){return console.error("Model availability check failed:",e),console.log("Error details:",{message:e.message,stack:e.stack}),!1}}async function s(e,r){try{const a=await(await ai.languageModel.create({systemPrompt:`What is the subject of the text below? Pick the best one:
    - Science
    - Math
    - Social Studies
    - Language Arts.
